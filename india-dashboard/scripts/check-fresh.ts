...
for (const id of ids) {
  const r = query("SELECT MAX(fetched_at) as max FROM data_points WHERE indicator_id = ?", [id]);
  console.log(id, "max_fetched:", r[0]?.max ?? "NULL");
  const cnt = query("SELECT COUNT(*) as n FROM data_points WHERE indicator_id = ?", [id]);
